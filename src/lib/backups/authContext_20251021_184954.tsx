...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Create singleton chat service when account is available (stateless pattern)
  const chatService = useMemo(() => {
    if (!supabase) return null;
    console.log('🔬 AuthContext: ========== CREATING CHAT SERVICE ==========');
    console.log('🔬 AuthContext: Account state:', { 
      hasAccount: !!account, 
    ...
    supabase.auth.getSession().then(({ data: { session }, error }) => {
      console.log('🔬 AuthContext: Session state during chatService creation:', {
        hasSession: !!session,
      ...
      console.log('🔬 AuthContext: ID MATCH CHECK:', {
        accountId: account?.id,
        sessionUserId: session?.user?.id,
        idsMatch: account?.id === session?.user?.id,
        CRITICAL: account?.id === session?.user?.id ? '✅ IDs MATCH' : '🔴 IDS MISMATCH!'
      });
    });
    
    console.log('🔬 AuthContext: Creating getAccount closure');
    console.log('🔬 AuthContext: Closure will capture account:', account?.id);
    
    const instance = new ChatService(supabase, () => {
      console.log('🔬 ChatService.getAccount() called, returning:', account?.id);
      return account;
    });
    
    // Expose for legacy imports that expect a global singleton
    ...
    console.log('✅ AuthContext: SimpleChatService instance created');
    return instance;
  }, [supabase, account]);
