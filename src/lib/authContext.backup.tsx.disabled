...
interface AuthContextType {
  ...
  checkUserExists: (phone?: string, email?: string) => Promise<{ exists: boolean; userData?: { id: string; name?: string; email?: string; phone?: string; profile_pic?: string; bio?: string; dob?: string; created_at: string; updated_at: string } | null; error: Error | null }>;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const sendPhoneVerification = async (phone: string) => {
    ...
    try {
      ...
      if (exists && userData) {
        console.log('sendPhoneVerification: ✅ Phone belongs to existing account:', userData.name);
  ...
  const verifyPhoneCode = async (phone: string, code: string) => {
    ...
    try {
      ...
      // If this phone belongs to an existing account, just sign in (no merging)
      if (exists && userData) {
        console.log('verifyPhoneCode: ✅ Phone belongs to existing account - signing in without merging');
        console.log('verifyPhoneCode: Using existing profile data:', userData.name);
        setUser(data.user);
        return { error: null, isExistingAccount: true };
      } else {
        console.log('verifyPhoneCode: 🆕 New phone number - creating new account');
        setUser(data.user);
        return { error: null, isExistingAccount: false };
      }
    } catch (error) {
  ...
  const verifyEmailCode = async (email: string, code: string) => {
    ...
    try {
      ...
      console.log('verifyEmailCode: Email verification successful, new auth user:', data.user.id);
          ...
          // SIMPLE: Update existing profile to use new auth user ID (no deletion)
          console.log('verifyPhoneCode: 🔄 Merging accounts by transferring data from old to new profile:', { 
            oldUserId: userData.id,
            newUserId: data.user.id,
            keepingData: { name: userData.name, avatar: userData.profile_pic, bio: userData.bio }
          });
          ...
          if (createError) {
            ...
            const { data: updateResult, error: updateError } = await supabase
              ...
              .update({
                ...
                phone: phone,
                bio: userData.bio,
                profile_pic: userData.profile_pic,  // PRESERVE AVATAR URL
                dob: userData.dob,
            ...
            if (updateError) {
              ...
              console.log('verifyPhoneCode: 🖼️ Avatar URL preserved:', updateResult[0]?.profile_pic);
            }
          }
          ...
          if (typeof window !== 'undefined') {
            window.__CONNECT_EXISTING_PROFILE__ = {
              ...
              profile_pic: null   // Don't cache avatar - always load fresh from database
            };
  ...
  const verifyEmailCode = async (email: string, code: string) => {
    ...
    try {
      ...
      // If this email belongs to an existing account, just sign in (no merging)
      if (exists && userData) {
        console.log('verifyEmailCode: ✅ Email belongs to existing account - signing in without merging');
        console.log('verifyEmailCode: Using existing profile data:', userData.name);
        setUser(data.user);
        return { error: null, isExistingAccount: true };
      } else {
        console.log('verifyEmailCode: 🆕 New email - creating new account');
        setUser(data.user);
        return { error: null, isExistingAccount: false };
      }
    } catch (error) {
  ...
  const checkUserExists = useCallback(async (phone?: string, email?: string) => {
    if (!supabase) {
      return { exists: false, userData: null, error: new Error('Supabase client not initialized') };
    }
      ...
      console.log('checkUserExists: SIMPLIFIED CHECK for existing account', { phone, email });
          ...
          if (existingProfile) {
            ...
            if (updateError) {
              ...
              console.log('verifyEmailCode: 🖼️ Avatar URL preserved:', updateResult[0].profile_pic);
            } else {
          ...
          if (typeof window !== 'undefined') {
            ...
            console.log('verifyEmailCode: ⚡ Profile data stored for INSTANT loading (multiple storage methods):', {
              ...
              storedProfileName: profileToStore.name,
              originalProfileId: userData.id,
              newAuthUserId: data.user.id,
              avatarWillLoadFresh: profileToStore.profile_pic === null,
  ...
  const checkUserExists = useCallback(async (phone?: string, email?: string) => {
    ...
    try {
        ...
        try {
          ...
          if (allProfiles && allProfiles.length > 0) {
            ...
            for (const profile of allProfiles) {
              if (profile.phone) {
                console.log(`🔍 Checking profile: "${profile.phone}" (${profile.name})`);
      ...
      if (data && data.length > 1) {
        ...
        console.log('checkUserExists: Using most recent account:', { id: userData.id, name: userData.name });
        return { 
          exists: true, 
          userData,
          error: null,
      ...
      console.log('checkUserExists: Result:', { exists, userData: userData ? { id: userData.id, name: userData.name, email: userData.email, phone: userData.phone } : null });
