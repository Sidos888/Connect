...
interface DisableEventChatModalProps {
  isOpen: boolean;
  onClose: () => void;
  listingTitle: string;
...
export default function DisableEventChatModal({
  isOpen,
  onClose,
  listingTitle,
  onConfirm,
}: DisableEventChatModalProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      console.error("DisableEventChatModal: Error confirming disable:", error);
      alert("Failed to disable event chat. Please try again.");
        ...
        <div className="flex-1 flex flex-col px-6 pb-6">
          <div className="flex-1 flex items-center justify-center">
            <p className="text-base text-gray-900 text-center">
              Are you sure you want to disable event chat for {listingTitle}?
            </p>
          </div>
          ...
          >
            {isProcessing ? "Disabling..." : "Disable Event Chat"}
          </button>
        </div>
      </div>
    </div>
