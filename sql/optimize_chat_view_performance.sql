-- Optimize chat_list_optimized view for better performance
-- This creates a more efficient version that should load in <500ms

-- Drop the existing view
DROP VIEW IF EXISTS chat_list_optimized CASCADE;

-- Create a more optimized view with better performance
CREATE OR REPLACE VIEW chat_list_optimized AS
WITH chat_participants_data AS (
  -- Get all participants for each chat (excluding current user)
  SELECT 
    cp.chat_id,
    json_agg(
      json_build_object(
